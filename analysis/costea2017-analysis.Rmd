...
## What fraction of each sample is `Genus_unclassified`?
...
all taxa have a positive measured abundance, so let's check that there are no
zeros for the mock taxa:
```{r}
st0 %>%
    filter(Taxon %in% mock$Taxon, Abundance == 0)
```
From now on, we will restrict to just the mock taxa,
```{r}
stm <- st0 %>%
