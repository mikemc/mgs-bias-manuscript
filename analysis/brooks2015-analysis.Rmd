...
ggplot_cb <- function(...) {
    ggplot2::ggplot(...) + scale_color_manual(values = colors.OkabeIto$Hex) + 
        scale_fill_manual(values = colors.OkabeIto$Hex)
    # ggplot2::ggplot(...) + 
...
# or try
# scale_colour_discrete <- function(...) scale_color_manual(..., values)
...
```{r}
main %>%
    select(Sample, Mixture_type, Num_species, Taxon, Expected) %>%
    spread(Taxon, Expected) %>%
    filter(Num_species > 1) %>%
    select(-Sample) %>%
    distinct() %>%
    group_by(Mixture_type) %>%
...
main %>%
    ...
    filter(!Expected) %>%
    ggplot_cb(aes(Prop, fill = as.factor(Num_species))) +
    geom_dotplot() +
    scale_x_log10() +
...
main0 %>%
    ...
    ggplot_cb(aes(Actual, Observed, color = Taxon)) + 
...
main0 %>%
    ...
    ggplot_cb(aes(Actual, CN_corrected, color = Taxon)) + 
    geom_quasirandom() +
    facet_grid(Mixture_type ~ .) +
    geom_rangeframe(sides = "bl", color= "black") +
...
p.pw <- ratios %>% 
    filter(Pair %in% cmb) %>%
    ggplot_cb(aes(Pair, Error, color = as.factor(Num_species))) +
...
```
Compute the predicted proportions
```{r}
main0 <- main0 %>%
...
```{r, fig.width = 6.5, fig.height = 6.5}
p <- ggplot_cb(pred, aes(logit(Predicted), logit(Observed), color = Taxon)) +
    geom_abline(intercept = 0, slope = 1, color = "grey") +
    ...
    coord_fixed() +
    scale_color_manual(values = colors.OkabeIto$Hex, labels = tax_labeller) + 
    theme(
...
# Panel A
p.A <- ggplot_cb(pred0, aes(Actual, Observed, color = Taxon)) +
    ...
    base_theme
# Panel B
p.B <- ggplot_cb(pred0, aes(Predicted, Observed, color = Taxon)) +
    geom_segment(data=ref_line, aes(x=x, xend=xend, y=y, yend=yend), 
        colour="grey") +
    geom_point() +
    geom_rangeframe(color = "black") +
...
p.C <- 
    ggplot_cb(pred0.odds, aes(x = Taxon, y = Observed / Actual,
    ...
    base_theme +
    tax_theme
p.D <- ggplot_cb(ratios0, aes(Pair, Error, color = as.factor(Num_species), 
        shape = Shape)) +
    # Reference line
    geom_hline(yintercept = 1) +
...
```{r, include = FALSE, eval = SAVE_FIGURES}
ggsave(file.path(fig_path, "brooks2015_4panel.pdf"),
    width = 6.5, height = 5.5, units = "in", scale = 4/3)
ggsave(file.path(fig_path, "brooks2015_4panel.png"),
    width = 6.5, height = 5.5, units = "in", scale = 4/3)
```
...
bias_steps <- bias %>% 
    ...
    mutate_at(vars(Genome_size, Copy_number), center_elts)
bias_steps
```

The estimated bias of the step is ....
- Extraction bias = Difference from Cells to DNA;
- PCR bias = Difference from DNA to PCR product
- Seq + bioinf bias = Difference from PCR product to unbiased

todo - consider adding Reads = 1 here.
...
Left panel: Estimated bias for each mixture experiment, plus an imagined
"Reads" mixture experiment that by definition has no bias.
```{r}
# Setup tibble for plotting
bias_steps.plot <- bias_steps %>%
    select(Taxon, Cells, DNA, PCR_product) %>%
    add_column(`Reads*` = 1) %>%
    gather("Mixture_type", "Bias", -Taxon) %>%
    mutate_at(vars(Mixture_type), str_replace, "_", " ") %>%
    mutate(Taxon = factor(Taxon, bias_steps$Taxon))
# Tibble for plotting a reference line corresponding to no bias
ref.left <- tibble(Mixture_type = factor(c("Cells", "Reads*"), 
        c("Cells", "DNA", "PCR product", "Reads*")),
    Bias = 1)
p.left <- ggplot(bias_steps.plot, aes(Mixture_type, Bias, color = Taxon)) +
    geom_path(data = ref.left, aes(group = Bias), color = "grey") +
    geom_path(aes(group = Taxon)) +
    geom_point() +
    scale_y_log10(limits = c(0.1, 5), labels = log_formatter) +
    scale_x_discrete(expand = c(0.07, 0.06)) +
    geom_rangeframe(color = "black") +
    scale_color_manual(values = colors.OkabeIto$Hex, labels = tax_labeller) + 
    labs(x = "Mixture type", y = "Efficiency / Gm. mean", 
        title = "Bias in each mixture type") +
...
Right panel: Predicted relative abundances through the workflow starting from
an even mixture of cells.

```{r}
# Tibble of the relative abundances through the workflow
ra_steps <- bias_steps %>%
    add_column(T0 = 1) %>%
    mutate(T1 = T0 * Extraction, T2 = T1 * PCR, T3 = T2 * Sequencing) %>%
    transmute(Taxon, Actual = T0, After_extraction = T1, After_PCR = T2,
        Observed = T3) %>%
    gather("Step", "Abundance", -Taxon) %>%
    mutate_at(vars(Step), str_replace, "_", " ") %>%
    mutate(Taxon = factor(Taxon, bias_steps$Taxon))
# Tibble for reference line
ref.right <- tibble(Step = factor(c("Actual", "Observed"), 
        c("Actual", "After extraction", "After PCR", "Observed")),
    Abundance = 1)
p.right <- ra_steps %>%
    ggplot(aes(Step, Abundance, color = Taxon)) +
    ...
    geom_rangeframe(color = "black") +
    scale_color_manual(values = colors.OkabeIto$Hex, labels = tax_labeller) + 
    labs(x = "Step in workflow", y = "Abundance / Gm. mean", 
...
```{r}
l <- get_legend(p.left + theme(legend.position = c(0.45, 0.55), 
...
```{r, include = FALSE, eval = SAVE_FIGURES}
ggsave(file.path(fig_path, "brooks2015_bias_workflow.png"),
    width = 7, height = 3, units = "in")
ggsave(file.path(fig_path, "brooks2015_bias_workflow.pdf"),
    width = 7, height = 3, units = "in")
ggsave(file.path(fig_path, "brooks2015_bias_workflow.svg"),
    width = 7, height = 3, units = "in", scale = 4/3)
```

Bias of individual steps appears as the multiplicative changes from one point
to the next (right panel); and the multiplicative differences between
experiments (left panel)

The (compositional) difference between the bias observed in different mixture
types provides an estimate of the bias of individual workflow steps. 

( and to
predict how the composition of a sample changes throughout the workflow.
)

### Alt. - just p.right, with copy number correction

```{r}
lvls.position <- c("Actual", "After extraction", "After PCR", "Read counts", 
            "CN corrected")
# Tibble of the relative abundances through the workflow
ra_steps <- bias_steps %>%
    add_column(T0 = 1) %>%
    ...
    transmute(Taxon, Actual = T0, After_extraction = T1, After_PCR = T2,
        Read_counts = T3, CN_corrected = T4) %>%
    mutate(Taxon = fct_reorder(Taxon, CN_corrected)) %>%
    gather("Position", "Abundance", -Taxon) %>%
    mutate_at(vars(Position), str_replace, "_", " ")
# Tibble for reference line
ref.right <- tibble(Position = factor(lvls.position, lvls.position), Abundance = 1)
p.right <- ra_steps %>%
    ggplot(aes(Position, Abundance, color = Taxon)) +
    geom_path(data = ref.right, aes(group = Abundance), color = "grey") +
    geom_path(aes(group = Taxon)) +
    geom_point() +
    scale_y_log10(breaks = c(0.1, 0.3, 1, 3, 10), labels = log_formatter) +
    # scale_y_log10(limits = c(0.1, 5), labels = log_formatter) +
    scale_x_discrete(expand = c(0.07, 0.06)) +
    geom_rangeframe(color = "black") +
    scale_color_manual(values = colors.OkabeIto$Hex, labels = tax_labeller) + 
    labs(x = "Position in the workflow", y = "Abundance / Gm. mean", 
        title = "Composition throughout the workflow") +
    guides(color = guide_legend(reverse = TRUE)) +
    base_theme
p.right + 
    theme(legend.position = "right")
...
Q: Would it be better to label the steps (the points in between the positions /
time points, as Extraction, PCR, Sequencing, CN correction), rather than the
workflow positions?

Also, should we change Observed to "After informatics" or "Assigned reads"
similar, since the cn-corrected abundances "Observed" could also be the 

```{r, include = FALSE, eval = SAVE_FIGURES}
ggsave(file.path(fig_path, "brooks2015_bias_workflow_1p.png"),
    width = 4.5, height = 3, units = "in")
ggsave(file.path(fig_path, "brooks2015_bias_workflow_1p.pdf"),
    width = 4.5, height = 3, units = "in")
```
...
setup
```{r}
lvls.steps <- c("Extraction", "PCR", "Sequencing", "CN correction")
bias_steps.plot <- bias_steps %>%
    transmute(Taxon, Extraction, PCR, Sequencing, 
        CN_correction = 1 / Copy_number) %>%
    mutate(Taxon = fct_reorder(Taxon, 
            Extraction * PCR * Sequencing * CN_correction)) %>%
    gather("Step", "Bias", -Taxon) %>%
    mutate_at(vars(Step), str_replace, "_", " ")
lvls.position <- c("Actual", "After extraction", "After PCR", "Read counts", 
            "CN corrected")
# Tibble of the relative abundances through the workflow
...
Alt left panel - show the estimated bias of each step
```{r}
# shared params
...
## Left panel
# Tibble for reference line
ref.left <- tibble(Step = factor(lvls.steps, lvls.steps), Bias = 1)
# make plot
p.left <- ggplot(bias_steps.plot, aes(Step, Bias, color = Taxon)) +
    geom_path(data = ref.left, aes(group = Bias), color = "grey") +
    geom_quasirandom(width = 0.2) +
    scale_y_log10(limits = ylimits, labels = log_formatter) +
    scale_x_discrete(expand = xexpand) +
    geom_rangeframe(color = "black") +
    scale_color_manual(values = colors.OkabeIto$Hex, labels = tax_labeller) + 
...
```{r, fig.width = 10, fig.height = 6.5}
ggplot_cb(cal, aes(Taxon, odds(Estimate) / odds(Actual), 
        color = as.factor(Num_species))) +
    geom_quasirandom() +
    scale_y_log10() + 
...
ggplot_cb(preds.slm, aes(.fitted, Observed, color = Taxon)) +
    geom_quasirandom() +
    facet_grid(Mixture_type ~ Model) +
    labs(c = "Model prediction")
```

The models are very similar, so we'll use the simpler model (with no intercept
term) from here on.
<<<<<<< Updated upstream
=======
...
ggplot_cb(preds, aes(logit(Predicted), logit(Observed), color = Taxon)) +
    geom_abline(intercept = 0, slope = 1, color = "grey") +
    geom_jitter(width = 0.1, height = 0) +
    geom_rangeframe(color = "black") + 
    facet_grid(Mixture_type ~ Model) +
...
```{r, include = FALSE, eval = SAVE_FIGURES}
ggsave(file.path(fig_path, "model_comparison.png"),
    width = 6.5, height = 6.5, units = "in")
ggsave(file.path(fig_path, "model_comparison.pdf"),
    width = 6.5, height = 6.5, units = "in")
```
